            ...
            <tr role="row" class="odd">
                <td>
                    <div class="img-container">
                        <img src="{{ asset($subCategory->sub_category_details['image']) }}"
                            alt="{{ $subCategory->name }}" height="100" width="100">
                    </div>
                </td>
                <td>{{ $subCategory->name }}</td>
                <td>{{ $subCategory->sub_category_details['category'] }}</td>
                <td>
                    <a href="{{ route($segment . '.' . 'sub-category-wise-brands.show', $subCategory->slug) }}">
                        <i class="fas fa-folder-open"></i>
                        {{ $subCategory->brands->count() }}
                    </a>
                </td>
                <td>
                ...
                <td class="text-right">
                    ...
                    <a href="{{ route($segment . '.' . 'sub-category.edit', $subCategory->id) }}"
                    ...
                    <a class="btn btn-link btn-danger btn-just-icon remove">
                        <form onsubmit="return confirm('Do you really want to delete?');"
                            action="{{ route($segment . '.' . 'sub-category.destroy', $subCategory->id) }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit">
                ...
                @include('Dashboard.Admin.Sub-Category.Partials.modal')
            </tr>
        @endforeach
    </tbody>
</table>
