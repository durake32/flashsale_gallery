...
<img src="{{ asset('Asset/Uploads/Products/' . $image) }}" width="100%" height="100%">
@endforeach
@endif
<div class="hs_bottom">
    ...
    <ul style="margin-top:50px; margin-right:15%;">
        <li>
            <a href="{{ route('product-details', $newProduct->slug) }}"><span class="fa fa-eye"
                    aria-hidden="true"></span></a>
        </li>
        <li>
            <form class="cart mt-3" role="form" action="{{ route('index.cart.store') }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <input type="hidden" id="number" name="quantity" value="1" />
                <input type="hidden" id="id" name="id" value="{{ $newProduct->id }}" />
                <button type="submit" class="AddtoCartbtn button alt" style="background-color:white;padding:6px"><span
